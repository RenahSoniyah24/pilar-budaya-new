...
import qs from 'qs';
import { coloumn_detail_iuran } from '../../../../Constant/DataColoumn';
import { formatDate, calculateAge } from '../../../../../src/Formatter/Text';
...
function IuranDetail(props) {
  ...
  const fetchData = async () => {
    ...
    let response = await getUserDetailService(identifier, 'iuran');
  ...
  const handleVerifikasi = async (val) => {
    setLoading(true);
    let response = await verifikasiService(identifier, val);
    if (response) {
      setLoading(false);
      ...
      <Admin>
        <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <div className="h4 d-flex align-items-center">
            <FaWallet size={20} className="mx-2" />
            Detail Iuran Pengguna
          </div>
        </div>
        ...
        <div className="table-responsive">
          <Table
            columns={coloumn_detail_iuran(handleDataModal, handleVerifikasi)}
